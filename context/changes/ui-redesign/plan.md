...
- [x] 5.1 Solution builds: `dotnet build`
- [x] 5.2 Test suite passes: `dotnet test`
- [x] 5.3 Full hook check: the entire `feature/test-plan` suite passes in the throwaway worktree
- [x] 5.4 Profile still carries all three `FormName`s
- [x] 5.5 `grep -rn "(MarkupString)" Components` matches only `Components/Notes/NoteEditor.razor`
- [x] 5.6 No `@onclick` on any static-SSR page
- [x] 5.7 Template leftovers (`text-secondary small`, `list-group`, `bi-*-nav-menu`) are gone
