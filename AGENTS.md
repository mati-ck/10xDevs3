...
- Application code obtains data through `UserScopedDbContextFactory`, **never** by injecting a `DbContext`. The bare `AppDbContext` registration exists only for infrastructure that must resolve it (DataProtection's key store, the startup migration service, the EF health check); a context obtained that way has no current user, so reads return nothing and writes throw — which presents as "the database is empty". `tests/10xNotes.Tests/DataAccessBoundaryTests.cs` fails the build if any type in the app assembly takes a `DbContext`.
