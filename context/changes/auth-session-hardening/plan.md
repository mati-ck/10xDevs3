...
#### 3. Model snapshot migration

**File**: `Migrations/<timestamp>_FreezeOwnerIdConcurrencyToken.cs` (new)

**Intent**: Record the model change so `has-pending-model-changes` stays clean and the snapshot matches the code.

**Contract**: Generated by `dotnet ef migrations add FreezeOwnerIdConcurrencyToken`. A concurrency token is EF metadata, not schema, so the migration is expected to carry no meaningful DDL — verify that before applying, and confirm it introduces no table (the RLS rule in `context/foundation/lessons.md` binds only when a table is created). Backward-compatible by construction, which the Coolify rollback story requires.
...
- Solution builds: `dotnet build 10xnotes.sln` → 0 warnings, 0 errors
- Tests pass, including the five new cases: `dotnet test`
- Model and snapshot agree: `dotnet ef migrations has-pending-model-changes` reports none
- Migration applies: `dotnet ef database update`

#### Manual Verification:

- The generated migration contains no table creation and no destructive DDL — read it before applying
- Supabase security advisor reports no new `rls_disabled` findings
...
One migration lands here, and it is expected to carry no schema change — a concurrency token is model metadata. Verify that before applying; if EF does emit DDL, read it and confirm it is additive, since a Coolify rollback restores the previous image but leaves the newer schema.
...
- [ ] 2.1 Solution builds: `dotnet build 10xnotes.sln` → 0 warnings, 0 errors
- [ ] 2.2 Tests pass, including the five new cases: `dotnet test`
- [ ] 2.3 Model and snapshot agree: `has-pending-model-changes` reports none
- [ ] 2.4 Migration applies: `dotnet ef database update`

#### Manual

- [ ] 2.5 The generated migration contains no table creation and no destructive DDL
- [ ] 2.6 Supabase security advisor reports no new `rls_disabled` findings
