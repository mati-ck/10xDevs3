...
- [x] 4.1 Solution builds: `dotnet build` — f544a67
- [x] 4.2 Test suite passes: `dotnet test` — f544a67
- [x] 4.3 Hook check: the `feature/test-plan` suite passes in the throwaway worktree — `NoteEditorTests` and `MaterialsDetailTests` included — f544a67
- [x] 4.4 `grep -rn "(MarkupString)" Components` still matches only `Components/Notes/NoteEditor.razor` — f544a67
- [x] 4.5 The number of `<button` elements in `Materials/Detail.razor` is unchanged — f544a67
- [x] 4.6 `<h1>Nie znaleziono materiału</h1>` is present verbatim in `Materials/Detail.razor` — f544a67
- [x] 4.7 NoteEditor has exactly one `div.form-text`, the counter, before the textarea — f544a67
- [x] 4.8 `btn-primary` in NoteEditor matches only the save button — f544a67
- [x] 4.9 `grep -rnE 'data-bs-(dismiss|toggle)="|IJSRuntime' Components` returns nothing — f544a67
- [x] 4.10 Both detail pages' scoped `.razor.css` files are removed — f544a67
...
- [x] 5.1 Solution builds: `dotnet build` — 6c03e82
- [x] 5.2 Test suite passes: `dotnet test` — 6c03e82
- [x] 5.3 Full hook check: the entire `feature/test-plan` suite passes in the throwaway worktree — 6c03e82
- [x] 5.4 Profile still carries all three `FormName`s — 6c03e82
- [x] 5.5 `grep -rn "(MarkupString)" Components` matches only `Components/Notes/NoteEditor.razor` — 6c03e82
- [x] 5.6 No `@onclick` on any static-SSR page — 6c03e82
- [x] 5.7 Template leftovers (`text-secondary small`, `list-group`, `bi-*-nav-menu`) are gone — 6c03e82
...
> P5 implementation notes (non-interactive run): manual rows 5.8–5.11 are left for a browser pass, and they are the last four of the run's twenty-nine manual rows (1.7–1.9, 2.9–2.15, 3.8–3.14, 4.11–4.18, 5.8–5.11), none of which an automated check can stand in for. Hook check 5.3 ran before the commit, as in P2–P4: a detached worktree at `/tmp/10xDevs3-hookcheck` on the P4 tip with `feature/test-plan` merged in (clean auto-merge, `Import.razor` included), then this phase's five working-tree files copied over — `feature/test-plan` touches none of them. `dotnet test`: 387 passed; worktree removed. On this branch `dotnet build` is clean and `dotnet test` is 355 passed. Contract additions: the password section's two fields stay plain `InputText`s (no `.input-icon`) — the artboard puts the lock icon only in the delete field; `div.field` rather than `div.field.mb-0`, since `.settings-card .field { margin-bottom: 0 }` has been in `app.css` since P1 (same reasoning as P4's title field); the danger zone's two paragraphs sit in a `div.settings-copy` (flex, 10px) so they read as one block instead of being pushed 18px apart by the card's gap; NotFound's `h3` and `p` sit in the copy `div` the other empty states use, so `.empty-state > div:not(.icon-tile)` gives them 6px; `Error.razor` gets `.system-card h3` as well as the planned `h2`, because Bootstrap's 24.5px h3 would otherwise dwarf the 16px h2 above it. New `app.css` rules: `.settings-intro`, `.settings-card > .alert`, `.settings-card form` (the 18px rhythm, since EditForm renders the form), `.settings-card .form-actions`, `.settings-card p`, `.settings-copy`, `.narrow-card p`, `.system-card` (+ `h2`, `h3`, `code`), `.empty-state h3`. Responsive pass, all CSS-only and inside the three existing breakpoints: `h1` and `.system-card code` get `overflow-wrap: anywhere` (a user title or a trace id has no spaces); `.seg` gets `max-width: 100%`; `--bs-modal-margin: 16px` moves into the `.modal` block rather than a new 576px query — Bootstrap caps the dialog at `--bs-modal-width` only from 576px up, so the margin is what keeps it off the edges below that, and above it the dialog is centred anyway; the 768px `.material-row` note column becomes `minmax(0, auto)` so a long status badge cannot widen the row; the 641px block gains `.pane-head { flex-wrap: wrap }`, `.pane-link { max-width: 100% }`, 18px padding for `.settings-card` / `.import-card` / `.narrow-card` / `.dropzone`, tighter `.empty-state` and `.pane-empty` padding, and a narrower `#blazor-error-ui`. The `.list-group` mapping the phase was to delete was already gone (P3 removed it), so check 5.7 passed unchanged. This note back-fills P4's `f544a67`, as P4 back-filled P3's; the P5 rows were back-filled with `6c03e82` by the implementation review (`reviews/impl-review.md`), which re-ran 5.1–5.3 on the branch tip: `dotnet build` clean, `dotnet test` 355 passed, hook check 387 passed.
