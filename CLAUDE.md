...
This is a learning repository for the **10xDevs** course. It does not yet contain application code; the foundation artifacts produced by the course skills live under `context/foundation/` (e.g. `shape-notes.md`, `prd.md`). The rest of the repo is configuration tracking the `@przeprogramowani/10x-cli` tool that pulls down course lesson artifacts (skills, prompts, rules).

- `.claude/.10x-cli-manifest.json` — manifest of installed 10x-cli artifacts (skills, prompts) with per-lesson tracking and content hashes. Treat this as managed by `10x-cli`; do not hand-edit.
- `.claude/skills/` — installed skill bundles fetched by `10x get`. The active 10x-cli tool profile is **Claude Code** (`~/.config/10x-cli/config.json` → `{"tool": "claude-code"}`), so artifacts land under `.claude/` (skills, prompts) and the sentinel block is written into this `CLAUDE.md`. Switching profiles via `10x get <ref> --tool <other>` will prompt to migrate or delete these.

## Working with 10x-cli

When the user asks about installing, authenticating, fetching lessons, switching AI tool profiles, or troubleshooting the CLI, defer to the installed skills rather than answering from memory:

- **Setup / install / re-auth** → invoke the `10x-cli-setup` skill. It fetches the live README from GitHub as the source of truth.
- **Daily usage** (`10x get`, `10x list`, `10x doctor`, `10x auth`, tool switching, artifact locations, errors) → invoke the `10x-cli-guide` skill.

Both skill files live under `.claude/skills/<name>/SKILL.md` and describe their own scope and rules.

## When new code arrives

Once the user starts adding application code (likely as part of course lessons), update this file with the relevant build / test / run commands and the high-level architecture. Until then, do not invent commands or structure that does not exist.
